...
Gm03 = open("G--%s-0.0075.off" % dss)
Gp03 = open("G-%s-0.0075.off" % dss)
vm, fm = read_off(Gm03)
vp, fp = read_off(Gp03)
...
def initConfig(snapshot,Wall):
    ...
    def GetPointOnSurf(s=0):
        while(True):
            ...
            res = (np.sin(x*(2*np.pi)/L)*np.cos(y*(2*np.pi)/L) + np.sin(z*(2*np.pi)/L)*np.cos(x*(2*np.pi)/L) + np.sin(y*(2*np.pi)/L)*np.cos(z*(2*np.pi)/L))
...
suffix = "NANB" + str(N_A) + "_abGsurf_50x50x50_"
if( len(sys.argv) <= 1 ):
