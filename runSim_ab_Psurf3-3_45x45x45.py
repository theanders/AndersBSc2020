...
Gm03 = open("P--%s-0.0075.off" % dss)
Gp03 = open("P-%s-0.0075.off" % dss)
vm, fm = read_off(Gm03)
vp, fp = read_off(Gp03)
...
def initConfig(snapshot,Wall):
    ...
    def GetPointOnSurf(s=0):
        while(True):
            ...
            res = (np.cos(x*(2*np.pi)/L) + np.cos(y*(2*np.pi)/L) + np.cos(z*(2*np.pi)/L))
...
suffix = "Psurf_NANB" + str(N_A) + "_ab_" + str(L) + "x" + str(L) +"x" + str(L) + "_"
if( len(sys.argv) <= 1 ):
